...
import {insertPhanCongSuaChua, deletePhanCongSuaChua, getListDanhSachPhanCongSuaChua} from '../../api/Api_KyThuat';
import { useIsFocused } from '@react-navigation/native';

const WIDTH = Dimensions.get('window').width;
const HEIGHT = Dimensions.get('window').height;

export function ModalPicker_PhanCong(props)
{
    ...
    useEffect(() => {
      if(isFocused){
        _getListDanhSachPhanCongSuaChua();
      } 
      return () => {
    ...
    const _getListNhanVien = async () => {
      getListNhanVien(global.url, username, password, searchKey, type).then(obj => {
    ...
    const _getListDanhSachPhanCongSuaChua = async () => {
      getListDanhSachPhanCongSuaChua(global.url, username, password, madon).then(obj => {
            if(obj.ResultCode === true){    
    ...
    _deleteItemById = async (manvxl) => {
      deletePhanCongSuaChua(global.url, username, password, madon, manvxl).then(obj => {
            if(obj.ResultCode === true){    
    ...
    const _insertPhanCongSuaChua = async () => {
      insertPhanCongSuaChua(global.url, username, password, madon, manv).then(obj => {
            if(obj.ResultCode === true){    
